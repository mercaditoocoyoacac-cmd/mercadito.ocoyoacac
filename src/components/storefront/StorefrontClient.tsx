...
export function StorefrontClient({
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      ([entry]) => setHeaderFixed(!entry.isIntersecting),
      { threshold: 0 }
    );
  ...
  useEffect(() => {
    const handleKey = (e: KeyboardEvent) => {
      if (e.key === "Escape") { setQuickViewProduct(null); setShowCartPopover(false); }
    };
    ...
    <>
      ...
      >
        <div className="mx-auto max-w-6xl px-4">
          ...
          {/* Search bar */}
          <div className="pb-2" ref={headerRef}>
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-xl border-2 border-gray-200 bg-gray-50 py-2.5 pl-9 pr-9 text-sm outline-none focus:border-[var(--accent)] focus:bg-white transition-all"
              />
              {searchQuery && (
                <button
                  type="button"
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="mt-2 flex flex-wrap gap-1.5 text-xs text-gray-400">
                ...
                {store.phone && (
                  <>
                    {store.address && <span className="text-gray-300">·</span>}
                    <a href={`tel:${store.phone}`} className="text-[var(--accent)] hover:underline">{store.phone}</a>
                  </>
                )}
...
function AddToCartInline({
  ...
  const handleAdd = async () => {
    ...
    setLoading(false);
    if (res.status === 401) {
      setMessage("Inicia sesión para agregar.");
      return;
    }
    if (!res.ok || !data?.ok) {
...
function QuickViewModal({
  ...
  return (
    ...
    >
      <div
        className="relative w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-t-2xl sm:rounded-2xl bg-white shadow-2xl animate-slide-up-sm"
      ...
      >
        ...
        <button
          type="button"
          onClick={onClose}
          className="absolute top-3 right-3 z-10 rounded-full bg-white/90 p-2 shadow-md border-none cursor-pointer hover:bg-white"
        >
          <svg className="h-5 w-5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
          </svg>
        ...
        <div className="p-5 space-y-4">
          ...
          {product.sellByWeight && (
            <div className="flex items-center gap-2 text-sm text-gray-500">
              <span className="rounded-lg bg-orange-100 px-2.5 py-1 text-xs font-medium text-orange-700">
                Venta por peso
              </span>
              <span className="text-xs text-gray-400">
                {product.minWeightGrams}g – {product.maxWeightGrams}g
              </span>
            </div>
          )}
          ...
          <div className="pt-2">
            {isServicios ? (
              store.phone ? (
                ...
                <span className="block w-full rounded-xl bg-gray-200 px-5 py-3.5 text-center text-sm font-medium text-gray-500">
