...
export default function AdminPromocionesPage() {
  ...
  const filteredPromotions = filterStoreId
    ? promotions.filter((p) => p.store.id === filterStoreId)
    : promotions;
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!storeId) { setError("Selecciona una tienda"); return; }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-[color:var(--muted)]">
            Gestiona promociones multi-producto de todas las tiendas
          </p>
        </div>
        <button
          type="button"
      ...
      {!showForm && (
        <div className="mt-4">
          <label className="block">
            <div className="text-sm font-medium mb-1">Filtrar por tienda</div>
            <select
              value={filterStoreId}
              onChange={(e) => setFilterStoreId(e.target.value)}
              className="rounded-md border border-[var(--border)] bg-white px-3 py-2 text-sm text-gray-900 outline-none focus:border-[var(--accent)]"
            >
        ...
        <form onSubmit={handleSubmit} className="mt-6 rounded-xl border border-[var(--border)] p-5 space-y-4">
          ...
          <label className="block">
            ...
            <select
              ...
              className="mt-1 w-full rounded-md border border-[var(--border)] bg-white px-3 py-2 text-sm text-gray-900 outline-none focus:border-[var(--accent)]"
              disabled={!!editingId}
            >
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <div className="text-sm font-medium">
                Productos incluidos * ({selectedProducts.size} seleccionados)
              </div>
              {products.length > 0 && (
            ...
            {!storeId ? (
              <p className="text-sm text-[color:var(--muted)]">Selecciona una tienda para ver sus productos.</p>
            ) : products.length === 0 ? (
              <p className="text-sm text-[color:var(--muted)]">Esta tienda no tiene productos creados.</p>
            ) : (
