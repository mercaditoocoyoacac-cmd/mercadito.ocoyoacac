...
export default async function StorefrontPage({
  ...
  return (
    <main className="mx-auto w-full max-w-6xl flex-1 px-4 py-10">
        ...
        {store.category === "SERVICIOS" ? (
          store.phone ? (
            ...
            <div className="inline-flex shrink-0 items-center gap-2 rounded-md bg-gray-200 px-4 py-2 text-sm font-medium text-gray-500">
              Teléfono no disponible para citas
            </div>
          )
        ) : (
          <Link
      ...
      {products.length === 0 ? (
        <div className="mt-8 rounded-xl border border-[var(--border)] bg-[var(--accent-soft)] p-8 text-center">
          <div className="mx-auto mb-4 h-16 w-16 rounded-full bg-[var(--accent)]/20 flex items-center justify-center">
          ...
          <h2 className="text-xl font-semibold">
            {store.category === "SERVICIOS" ? "Aún no hay servicios publicados" : "Aún no hay productos"}
          </h2>
          <p className="mt-2 text-sm text-[color:var(--muted)] max-w-md mx-auto">
            {store.category === "SERVICIOS"
              ? "Este negocio aún no ha publicado sus servicios. Contáctalos para más información."
              : "Vuelve pronto."}
          </p>
        </div>
      ) : store.category === "SERVICIOS" ? (
        <div className="mt-8">
            ...
            {products.map((product: typeof products[number]) => (
              ...
              >
                <div className="flex items-start gap-3">
                  {product.imageUrl && (
                    <div className="h-14 w-14 shrink-0 overflow-hidden rounded-lg bg-[var(--accent-soft)]">
                      <img
          ...
          <div className="mt-6 rounded-xl border border-[var(--border)] bg-[var(--accent-soft)] p-6 text-center">
            <p className="text-sm text-[color:var(--muted)]">
              ¿Te interesa algún servicio? Agenda tu cita por WhatsApp
            </p>
